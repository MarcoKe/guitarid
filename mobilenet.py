...
for i in range(15):
    ...
    imshow(out, title=[class_names[x] for x in preds], name=str(i) + '.jpg')
print("input ", inputs[0], inputs[0].min(), inputs[0].max())

### save onnx

torch_input = torch.randn(1, 3, 224, 224)
onnx_program = torch.onnx.export(model, torch_input, "guitarid_model_v0.0.1.onnx", opset_version=7)
